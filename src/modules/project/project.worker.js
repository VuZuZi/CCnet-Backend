...
export const processViewSync = async (job) => {
  console.log(`[Worker] Starting job ${job.name}: Đang đồng bộ View an toàn...`);
  ...
  try {
    ...
    do {
      const result = await redisClient.scan(cursor, 'MATCH', 'project:*:views', 'COUNT', 500);
      cursor = result[0];
      allKeys.push(...result[1]);
    } while (cursor !== '0');

    if (allKeys.length === 0) return { success: true, message: 'Không có lượt view mới.' };
    ...
    for (const chunk of keyChunks) {
      ...
      if (updates.length > 0) {
        ...
        try {
          ...
          console.error(`[Worker] DB BulkWrite Lỗi! Đang rollback view lại Redis...`);
          const rbPipeline = redisClient.pipeline();
          updates.forEach(u => rbPipeline.incrby(`project:${u.projectId}:views`, u.val));
    ...
    console.log(`[Worker] Hoàn tất. Đã đồng bộ an toàn view cho ${totalSynced} dự án.`);
...
export const initProjectWorkers = () => {
  try {
    ...
    jobQueue.registerWorker('project-maintenance', processViewSync, {
      concurrency: 1,
    });
    ...
    console.log('[Worker] Project workers đã khởi chạy (Queue: project-maintenance)');
  } catch (error) {
