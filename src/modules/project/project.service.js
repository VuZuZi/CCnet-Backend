...
class ProjectService {
  ...
  _extractKnownCity(value) {
    ...
    const knownCities = [
      "ha noi", "hanoi", "ho chi minh", "tp hcm", "tp.hcm",
      "da nang", "can tho", "hai phong", "nha trang",
      "quang ninh", "hue",
    ];
  ...
  _getVolunteerProgress(project) {
    ...
    const target = Number(
      ...
      Array.isArray(project?.volunteerRoles)
        ? project.volunteerRoles.reduce((sum, role) => sum + Number(role?.quantity || 0), 0)
        : 0
    );
  ...
  _computeUrgencySignals(project) {
    ...
    const closedStatuses = new Set([
      "COMPLETED", "CLOSED", "CANCELLED", "COMPLETED_SUCCESSFULLY", "COMPLETED_PARTIAL",
    ]);
    ...
    const isFundedProject = String(project?.projectType || "").toUpperCase() === "FUNDED";
    const needsVolunteers = Boolean(
      project?.needsVolunteers || String(project?.projectType || "").toUpperCase() === "VOLUNTEER_ONLY"
    );
    ...
    if (!isOpen) {
      return { score: 0, isUrgent: false, manualUrgent, autoUrgent: false, reasons: [], daysLeft };
    }
    ...
    if (typeof daysLeft === "number") {
      if (daysLeft <= 3) {
        score += 40; reasons.push("Sắp hết hạn");
      } else if (daysLeft <= 7) {
        score += 25; reasons.push("Thời hạn đang rất gần");
      } else if (daysLeft <= 14) {
        score += 10;
      }
    }
    ...
    return { score, isUrgent: isUrgentEffective, manualUrgent, autoUrgent, reasons: reasons.slice(0, 3), daysLeft };
  }
  ...
  _decorateProjectUrgency(project) {
    ...
    return {
      ...
      urgencyMeta: { score: urgency.score, reasons: urgency.reasons, daysLeft: urgency.daysLeft },
    };
  }

  _extractUserSkillSet(user) {
    if (!Array.isArray(user?.skills)) return new Set();
    return new Set(user.skills.map((skill) => this._normalizeText(skill)).filter(Boolean));
  }
  ...
  _buildFeaturedReasonList(project, context = {}) {
    const reasons = [];
    const { userCity = "", supportedCategorySet = new Set(), appliedCategorySet = new Set(), followedProjectIds = new Set(), followedOrganizerIds = new Set(), userSkillSet = new Set() } = context;
    ...
    if (userCity && projectCity && userCity === projectCity) reasons.push("Gần khu vực của bạn");
    if (supportedCategorySet.has(project?.category)) reasons.push("Phù hợp lĩnh vực bạn từng tham gia");
    else if (appliedCategorySet.has(project?.category)) reasons.push("Cùng danh mục bạn từng quan tâm");
    if (followedProjectIds.has(projectId)) reasons.push("Bạn đã theo dõi dự án này");
    if (organizerId && followedOrganizerIds.has(organizerId)) reasons.push("Đến từ organizer bạn đang theo dõi");
    if (urgency.isUrgent) reasons.push("Dự án đang cần hỗ trợ gấp");
    if (daysLeft !== null && daysLeft >= 0 && daysLeft <= 7) reasons.push("Sắp hết hạn kêu gọi");

    if (fundingProgress >= 0.85 && fundingProgress < 1) reasons.push("Chỉ còn ít nữa là đạt mục tiêu gây quỹ");
    else if (fundingProgress >= 0.7 && fundingProgress < 0.85) reasons.push("Đang gần chạm mốc gây quỹ");

    if (project?.needsVolunteers) {
      if (volunteerProgress >= 0.65 && volunteerProgress < 1) reasons.push("Đang gần đủ đội ngũ tình nguyện");
      else if (volunteerProgress < 0.35) reasons.push("Đang cần thêm tình nguyện viên");
      if (skillMatchScore >= 8) reasons.push("Kỹ năng của bạn khá phù hợp");
    }

    if (project?.stats?.viewCount > 300) reasons.push("Được cộng đồng quan tâm nhiều");
    return reasons.slice(0, 3);
  }

  _scoreFeaturedProject(project, context = {}) {
    const { user = null, followedProjectIds = new Set(), supportedCategorySet = new Set(), appliedCategorySet = new Set(), followedOrganizerIds = new Set(), userCity = "", userSkillSet = new Set() } = context;
    ...
    if (user && Array.isArray(user?.skills) && user.skills.length > 0 && project?.needsVolunteers) {
      score += 2;
    }
    return score;
  }
  ...
  _roundRobinByOrganizer(projects = []) {
    ...
    for (const project of projects) {
      const organizerId = toIdString(project?.organizerId);
      const key = organizerId || `__no_org__:${project?._id?.toString?.() || Math.random()}`;
  ...
  _rebalanceExploreProjects(projects = [], followedOrganizerIds = []) {
    ...
    for (const project of projects) {
      const organizerId = toIdString(project?.organizerId);
      if (organizerId && followedSet.has(organizerId)) followedProjects.push(project);
  ...
  async syncVolunteerOnlyProjectStatus(project) {
    ...
    const shouldTrySync = project.projectType === PROJECT_TYPE.VOLUNTEER_ONLY && project.status === PROJECT_STATUS.RECRUITING;
  ...
  async syncVolunteerOnlyProjectsStatus(projects = []) {
    return Promise.all(projects.map((project) => this.syncVolunteerOnlyProjectStatus(project)));
  }
  ...
  buildWorkspaceProject(project) {
    ...
    if (milestones.length > 0) {
      const activeIndex = milestones.findIndex(
        (m) => m.status === MILESTONE_STATUS.PROCESSING || m.status === MILESTONE_STATUS.PENDING
      );
      const selectedIndex = activeIndex === -1 ? milestones.length - 1 : activeIndex;
  ...
  async getFeaturedProjects(userId = null) {
    ...
    if (!userId) {
      ...
      return [{
        ...
        recommendationMeta: {
          ...
          reasons: selected.recommendationMeta?.reasons?.length > 0 ? selected.recommendationMeta.reasons : ["Dự án nổi bật đang được quan tâm"],
        },
      }];
    }
    ...
    const [followedProjectsRaw, volunteerApplicationsRaw, followedUsersRaw] = await Promise.all([
    ...
    const followedProjectIds = new Set((followedProjectsRaw || []).map((row) => toIdString(row?.projectId)).filter(Boolean));
    const followedOrganizerIds = new Set((followedUsersRaw || []).map((row) => toIdString(row?.followingId)).filter(Boolean));
    const appliedCategorySet = new Set((volunteerApplicationsRaw || []).map((row) => row?.opportunityId?.category).filter(Boolean));
    const supportedCategorySet = new Set((volunteerApplicationsRaw || []).filter((row) => String(row?.status || "").toUpperCase() === "APPROVED").map((row) => row?.opportunityId?.category).filter(Boolean));
    ...
    const rankingPool = followedOrganizerProjects.length > 0 ? followedOrganizerProjects : candidates;

    const scored = rankingPool
      .map((project) => {
        const score = this._scoreFeaturedProject(project, { user, followedProjectIds, supportedCategorySet, appliedCategorySet, followedOrganizerIds, userCity, userSkillSet });
        const reasons = this._buildFeaturedReasonList(project, { user, followedProjectIds, supportedCategorySet, appliedCategorySet, followedOrganizerIds, userCity, userSkillSet });
        return { project: enrichProject(project, score, reasons), score };
      })
      .sort((a, b) => b.score - a.score);

    return scored.length > 0 ? [scored[0].project] : [enrichProject(candidates[0], 0, [])];
  }
  ...
  async getVolunteerProjects() {
    ...
    return (syncedProjects || []).map((project) => this._decorateProjectUrgency(project));
  }
  ...
  async getExploreProjects(queryParams, userId = null) {
    ...
    if (userId) {
      const followedUsersRaw = await this.followRepository?.findFollowingUsers?.(userId, 200, null).catch(() => []);
      followedOrganizerIds = (followedUsersRaw || []).map((row) => toIdString(row?.followingId)).filter(Boolean);
    }

    if (organizerScope === "FOLLOWED" && followedOrganizerIds.length === 0) {
      return { projects: [], pagination: buildPagination(0, safePage, safeLimit) };
    }
    ...
    if (organizerScope === "FOLLOWED") filter.organizerId = { $in: followedOrganizerIds };
    ...
    const result = await this.projectRepository.findAllProjects({
      filter, skip: 0, limit: poolLimit, sortType: sort, textSearch
    });

    const syncedProjects = await this.syncVolunteerOnlyProjectsStatus(result.projects || []);
    const rebalanced = this._rebalanceExploreProjects(syncedProjects, followedOrganizerIds);
    const rankedProjects = organizerScope === "FOLLOWED" ? rebalanced.followedProjects : rebalanced.merged;
    const pagedProjects = rankedProjects.slice(skip, skip + safeLimit).map((project) => this._decorateProjectUrgency(project));
  ...
  async getWorkspaceProjects(organizerId, queryParams) {
    ...
    const result = await this.projectRepository.findOrganizerProjects({ organizerId, status, sortType: sort, skip, limit: safeLimit });
    const syncedProjects = await this.syncVolunteerOnlyProjectsStatus(result.projects || []);

    return {
      projects: syncedProjects.map((project) => this._decorateProjectUrgency(this.buildWorkspaceProject(project))),
      pagination: buildPagination(result.total, safePage, safeLimit),
    };
  }
  ...
  async createDraftProject(organizerId, projectData) {
    const coverPayload = Array.isArray(projectData.coverMedia)
      ...
      : projectData.coverMedia ? [projectData.coverMedia] : [];
  ...
  async updateDraftProject(projectId, organizerId, updateData) {
    ...
    let { deletedDocumentIds, coverMedia, documents, ...finalUpdateData } = updateData;

    if (finalUpdateData.projectType === PROJECT_TYPE.VOLUNTEER_ONLY || (!finalUpdateData.projectType && existingProject.projectType === PROJECT_TYPE.VOLUNTEER_ONLY)) {
    ...
    try {
      const updatedProject = await this.transactionManager.runInTransaction(async (session) => {
        ...
        if (finalUpdateData.milestones) {
          finalUpdateData.milestones = this._applyMilestoneSmartDefaults(finalUpdateData.milestones, fallbackLocation);
        }
