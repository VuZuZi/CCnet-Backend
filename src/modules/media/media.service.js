...
class MediaService {
  ...
  async _processImage(buffer) {
    try {
      ...
      throw new AppError(
        "Định dạng ảnh không hợp lệ hoặc file bị lỗi cấu trúc",
        422,
      );
    }
  }
  ...
  async _generateBlurHash(buffer) {
    try {
      ...
      console.error(
        "[CTO Warning] BlurHash lỗi, trả về null để không đứt luồng chính:",
        error,
      );
      return null;
    }
  ...
  async uploadMultiple(files, userId, context = "post") {
    ...
    for (const file of files) {
      try {
        ...
        throw new AppError(
          `Tải lên hình ảnh thất bại trong quá trình xử lý`,
          500,
        );
      }
    }
  ...
  async deleteMultiple(publicIds) {
    ...
    const promises = publicIds.map((id) =>
      this.cloudinary.uploader.destroy(id),
    );
